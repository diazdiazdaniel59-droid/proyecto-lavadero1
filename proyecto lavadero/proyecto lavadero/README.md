...
## Publicar en Vercel (evitar error 404)

Si ves **404: NOT_FOUND**, casi siempre es configuración o archivos no subidos.

### 1. Sube todos los archivos a GitHub

Asegúrate de que **`index.html`** y **`vercel.json`** estén en la raíz del repo (no solo el README):

```bash
git add index.html vercel.json README.md LICENSE .gitignore
git commit -m "fix: configuración Vercel y app estática"
git push
```

### 2. Ajustes en el panel de Vercel

En tu proyecto → **Settings → General → Build & Development Settings**:
...
El archivo `vercel.json` del repo fuerza el sitio estático y redirige rutas a `index.html`.
